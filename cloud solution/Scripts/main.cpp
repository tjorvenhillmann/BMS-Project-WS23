...
void setup() {
  // The setup uses functions from the influsdb.cpp file for better overview

  Serial.begin(115200);

  // Create the client with the set credentials in the influxdb.cpp file 
  client = createNewClient();

  // Try to connect to wifi 
  wifiConnect();

  // Check if the database server connection is valid and the server is responding properly
  checkServerConnection(client);
}
